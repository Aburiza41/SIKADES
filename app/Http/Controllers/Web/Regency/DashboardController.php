...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        foreach ($trainingLevels as $trainingLevel) {
            ...
            $trainings[$trainingLevel->title] = Official::whereIn('village_id', $villages->pluck('id'))
                // ->where('status', 'validasi')
        ...
        foreach ($organizationLevels as $organizationLevel) {
            ...
            $organizations[$organizationLevel->name] = Official::whereIn('village_id', $villages->pluck('id'))
            // ->where('status', 'validasi')
        ...
        foreach ($positions as $position) {
            $jabatan[$position->name] = Official::whereIn('village_id', $villages->pluck('id'))
                // ->where('status', 'validasi')
        ...
        $officials = Official::with(['village.district.regency', 'addresses', 'contacts', 'identities', 'studies', 'positions.position', 'officialTrainings', 'officialOrganizations', 'position_current.position'])
            ...
            ->when($villages, function ($query) use ($villages) {
                $query->whereHas('village', function ($q) use ($villages) {
                    $q->whereIn('village_id', $villages->pluck('id'));
                });
            })
