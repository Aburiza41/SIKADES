...
export default function Actions({ row, onEdit, onDelete, onView, onPrint, role }) {
    // console.log(row);
    return (
        <div className="flex space-x-2">
            ...
            {(row?.status === "daftar" || row?.status === "tolak") && (
                <>
                    <button onClick={() => onEdit(row, role)} className="text-yellow-500 hover:text-yellow-700">
                        <FiEdit size={16} />
                        </button>
                    <button onClick={() => onDelete(row, role)} className="text-red-500 hover:text-red-700">
                        <FiTrash size={16} />
                    </button>
                </>
            )}
