...
export default function OrganizationsForm({
  organizations = [], // Daftar semua organisasi untuk dropdown
  setOrganizations = () => {},
  officialOrganizations = [], // Data organisasi terkait pejabat
  setOfficialOrganizations = () => {},
}) {
  // Debugging
  console.log("organizations:", organizations);
  console.log("officialOrganizations:", officialOrganizations);
  ...
  const openAddDataModal = (index = null) => {
    if (index !== null) {
      ...
      setSelectedOrganization({
        ...
        label: organization.nama || organization.organization_title, // Gunakan nama atau organization_title
      });
      setFormData({
        ...
        doc_scan: organization.doc_scan || null,
      });
      setEditIndex(index);
    } else {
  ...
  const handleFileUpload = (e) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      setFormData((prev) => ({ ...prev, doc_scan: file }));
    }
  };
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1 flex items-center">
                ...
                Formulir ini digunakan untuk mengisi riwayat organisasi pejabat
                desa.
              </p>
            </div>
          </div>
                ...
                officialOrganizations.map((row, index) => (
                  ...
                  >
                    <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-800">
                      {row.organization_title || row.nama || "-"}
                    </td>
                    <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-800">
                      {row.nama || "-"}
                    </td>
                    ...
                    <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-800">
                      {row.doc_scan ? (
                        row.doc_scan === "exists" ? (
                          <span className="text-gray-400">Dokumen tersedia</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  ...
                  >
                    ...
                    <motion.div variants={inputVariants}>
                      <Select
                        ...
                        onInputChange={setNewOrganizationInput}
                        onKeyDown={(event) => {
                          if (
                            event.key === "Enter" &&
                            newOrganizationInput.trim()
                          ) {
                            handleAddNewOrganization(newOrganizationInput);
                  ...
                  >
                    <div className="text-sm text-gray-600 mt-2">
                      ...
                      {calculateDuration(formData.mulai, formData.selesai)}
                    </div>
                  </motion.div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {formData.doc_scan && (
                        <button
                          onClick={() =>
                            setFormData((prev) => ({ ...prev, doc_scan: null }))
                          }
                    ...
                    {formData.doc_scan && (
                      <div className="mt-2 flex items-center text-sm text-gray-600">
                        ...
                        <span>
                          {typeof formData.doc_scan === "string"
                            ? "Dokumen tersedia"
                            : formData.doc_scan.name}
                        </span>
                        {formData.doc_scan instanceof File && (
                          <span className="ml-2 text-gray-400">
                            ({(formData.doc_scan.size / 1024).toFixed(1)} KB)
                          </span>
                        )}
                      </div>
                    )}
