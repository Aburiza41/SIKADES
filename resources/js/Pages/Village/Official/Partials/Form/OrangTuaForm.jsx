...
export default function OrangTuaForm({ orang_tua = [], setOrangTua = () => {} }) {
  ...
  const initializeFormData = () => {
    ...
    // Jika ada data orang tua yang dikirim, gunakan data tersebut
    if (orang_tua && orang_tua.length > 0) {
      orang_tua.forEach(parent => {
        if (parent.jenis && ['ayah', 'ibu'].includes(parent.jenis)) {
          defaultData[parent.jenis] = { ...defaultData[parent.jenis], ...parent };
        }
      });
    }
  ...
  const renderParentForm = (parentType) => {
    ...
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="space-y-6 mb-8 p-6 border rounded-lg bg-white shadow-sm"
      >
        ...
        <div className="mt-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="form-group md:col-span-2">
              ...
              <input
                type="text"
                value={parentData.kode_pos}
                onChange={(e) => handleChange(e, parentType, 'kode_pos')}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
                placeholder="12345"
              />
            </div>
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 pb-6 border-b border-gray-200">
