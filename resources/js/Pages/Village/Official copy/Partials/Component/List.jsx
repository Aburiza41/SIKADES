...
export default function List({
    ...
    // Fungsi untuk mendapatkan jabatan terakhir dan sisa waktu menjabat
    const getPositionInfo = (positions) => {
        if (!positions || positions.length === 0) return { position: "-", remainingTime: "-" };

        // Urutkan berdasarkan tanggal mulai terbaru
        const sortedPositions = [...positions].sort((a, b) =>
            new Date(b.mulai) - new Date(a.mulai)
        );

        const latestPosition = sortedPositions[0];
        const positionName = latestPosition.position?.name || "-";

        // Hitung sisa waktu jika ada tanggal selesai
        let remainingTime = "-";
        if (latestPosition.selesai) {
            const endDate = new Date(latestPosition.selesai);
            const today = new Date();
            const diffTime = endDate - today;
            const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));

            if (diffDays > 0) {
                remainingTime = `${diffDays} hari`;
            } else {
                remainingTime = "Masa jabatan berakhir";
            }
        }

        return { position: positionName, remainingTime };
    };
    ...
    const renderSortableHeader = (field, label) => {
        ...
        return (
            <th
                className="border cursor-pointer hover:bg-gray-100 p-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
        ...
        <div className="bg-white rounded-lg shadow border border-gray-200 overflow-hidden">
            ...
            <div className="px-6 py-4 border-b border-gray-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex gap-2 w-full md:w-auto justify-end">
                    ...
                    <motion.a
                        as={Link}
                        href={route("village.official.create")}
                ...
                <table className="w-full divide-x divide-gray-200 text-sm border">
                    <thead className="bg-gray-50 border">
                        <tr>
                            ...
                            <th className="border px-2 py-1 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Jabatan</th>
                            <th className="border px-2 py-1 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sisa Jabatan</th>
                    ...
                    <tbody className="bg-white divide-x divide-gray-200 border">
                        {loading ? (
                            <tr>
                                <td colSpan="8" className="border px-2 py-2 text-center text-sm text-gray-500">
                                    Memuat data...
                                </td>
                            </tr>
                        ) : officials?.data?.length > 0 ? (
                            officials.data.map((row, index) => {
                                const { position, remainingTime } = getPositionInfo(row.positions);
                                return (
